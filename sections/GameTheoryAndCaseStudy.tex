...
The IC condition decomposes into three interpretable components:

\begin{itemize}
    \item $(\tau_{\text{val}} + 2\sigma)$: the \textbf{total economic exposure}---the validation fee plus twice the stake. This captures the total amount ``at risk'' that amplifies the value of being on the right side of the consensus.
    \item $(P_{\text{match}} - \frac{1}{2})$: the \textbf{informational advantage} of truthful effort over random guessing. The larger this gap, the more a truthful validator benefits relative to a shirker.
    \item $c_u$: the \textbf{effort cost} that must be overcome. The IC condition states that the expected economic advantage of being informed must exceed the cost of becoming informed.
\end{itemize}
...
\vspace{1mm}
\noindent\textbf{Interpretation: Supply-Side Self-Screening.}
The threshold $\bar{q}_p$ implements an elegant self-screening mechanism without centralized curation.
Producers with quality below $\bar{q}_p$ know that they are likely to fail validation (low $V_p$), making promotion unprofitable; they rationally abstain.
Producers above $\bar{q}_p$ expect to pass validation and earn subscription revenue; they rationally promote.
The market thus \textit{self-selects} for quality: the very act of paying promotion and validation fees serves as a credible signal of competence, because only agents who expect to pass can afford the risk.
