...
\begin{proof}
Truthful effort strictly dominates free-riding iff $\mathbb{E}[U \mid e=1] > \mathbb{E}[U \mid e=0]$. Substituting the expected payoffs from Equations~\eqref{eq:u_truthful} and~\eqref{eq:u_shirk}, we require:
\begin{align}
&P_{\text{match}} \cdot R_{\min} - (1 - P_{\text{match}}) \cdot \sigma - c_u > \frac{1}{2}(R_{\min} - \sigma). \nonumber
\end{align}
We expand the left-hand side by distributing $(1 - P_{\text{match}})$ and collecting terms involving $P_{\text{match}}$:
\begin{align}
\Longleftrightarrow\quad & P_{\text{match}} \cdot R_{\min} + P_{\text{match}} \cdot \sigma - \sigma - c_u > \frac{1}{2} R_{\min} - \frac{1}{2}\sigma. \nonumber
\end{align}
Rearranging to isolate $c_u$ on the right and grouping the terms involving $P_{\text{match}}$ and the constant $\frac{1}{2}$:
\begin{align}
\Longleftrightarrow\quad & P_{\text{match}} (R_{\min} + \sigma) - \frac{1}{2}(R_{\min} + \sigma) > c_u. \nonumber
\end{align}
Factoring out $(R_{\min} + \sigma)$---the total economic exposure per validation---yields:
\begin{align}
\Longleftrightarrow\quad & (R_{\min} + \sigma)\Big(P_{\text{match}} - \frac{1}{2}\Big) > c_u. \label{eq:ic_derivation}
\end{align}
The key insight of this derivation is that the IC condition reduces to a product of two terms: the \textit{total economic stake} $(R_{\min} + \sigma)$ and the \textit{informational edge} $(P_{\text{match}} - \frac{1}{2})$ that truthful effort provides over random guessing.
Since $R_{\min} = \tau_{\text{val}} + \sigma$ (the per-capita reward under unanimous agreement), we obtain $R_{\min} + \sigma = \tau_{\text{val}} + 2\sigma$, yielding the stated condition~\eqref{eq:ic}.
\end{proof}
...
The proof strategy is to show that the consumer's expected payoff from subscription is (1) monotonically increasing in producer quality, (2) negative at the low end and positive at the high end, and therefore (3) crosses zero exactly once, defining a unique threshold.

\textit{Step 1: Monotonicity.}
We must show that better producers yield higher posterior expected utility for the consumer---i.e., that the validation signal $Z$ is not only informative but \textit{monotonically} informative.
...
The MLRP implies that the posterior distribution $f(\cdot \mid Z = 1)$ under the higher-quality producer first-order stochastically dominates that under the lower-quality one---intuitively, observing $Z = 1$ from a better producer is ``stronger good news'' than from a weaker one.
Since $V_c(\cdot)$ is strictly increasing (higher quality $\Rightarrow$ higher consumer utility), the expected utility under stochastic dominance is strictly higher:
$\mathbb{E}[V_c \mid Z = 1, q_p'] > \mathbb{E}[V_c \mid Z = 1, q_p]$.
This establishes that the consumer's expected payoff from subscription is strictly increasing in the true quality of the producer being evaluated.

\textit{Step 2: Boundary behavior.}
We verify that the payoff is negative at one extreme and positive at the other, ensuring the threshold exists.
When $q_p \to \frac{1}{2}$ (minimal quality), the producer's output is near-random. Even conditional on $Z = 1$, the posterior assigns substantial mass to low-quality regions (because $V(\frac{1}{2}) > 0$, so ``pass'' signals still occur even for near-random producers). The posterior expected utility is low, bounded by $V_c(\frac{1}{2}) + \epsilon$ for small $\epsilon$, and the net payoff $\Pi_c < 0$ since $V_c(\frac{1}{2})$ is small (the producer is barely better than random) while $C_c$ is substantial.

When $q_p \to 1$ (perfect quality), the pass probability $V(q_p) \to 1$, the posterior concentrates near $q_p \approx 1$, and $\mathbb{E}[V_c \mid Z = 1] \to V_c(1) \gg C_c$, giving a strictly positive net payoff. This reflects the intuition that subscribing to a near-perfect producer is clearly worthwhile regardless of cost.

\textit{Step 3: Existence and uniqueness of the threshold.}
Combining Steps 1 and 2: the posterior expected utility is a strictly increasing, continuous function of $q_p$ that transitions from negative (at $q_p \to \frac{1}{2}$) to positive (at $q_p \to 1$).
By the intermediate value theorem, there exists a unique crossing point $\bar{q}_c$ where $\Pi_c = 0$.
By strict monotonicity, $\Pi_c \geq 0 \iff q_p \geq \bar{q}_c$, giving the stated threshold policy.
\end{proof}
