...
The proof proceeds in three steps: we show that the consumer's expected payoff is (1)~monotonically increasing in the \textit{true} producer quality $q_p$, (2)~negative at the low end and positive at the high end, and therefore (3)~crosses zero exactly once, defining a unique threshold.

\textit{Step 1: Monotonicity.}
We show that higher producer quality yields higher posterior expected utility for the consumer.
...
The MLRP implies that the posterior distribution $f(\cdot \mid Z = 1)$ under $q_p'$ first-order stochastically dominates that under $q_p$: a pass signal from a higher-quality producer concentrates posterior mass on higher quality levels.
...
When $q_p \to \frac{1}{2}$ (minimal quality), the producer's output is near-random. Even conditional on $Z = 1$, the posterior assigns substantial mass to low-quality regions (since $V(\frac{1}{2}) > 0$, pass signals still occur for near-random producers). The posterior expected utility is bounded above by $V_c(\frac{1}{2}) + \epsilon$ for small $\epsilon$, and $\Pi_c < 0$ since $V_c(\frac{1}{2})$ is small while $C_c$ is substantial.

When $q_p \to 1$ (perfect quality), $V(q_p) \to 1$, the posterior concentrates near $q_p \approx 1$, and $\mathbb{E}[V_c \mid Z = 1] \to V_c(1) \gg C_c$, giving $\Pi_c > 0$.
...
By strict monotonicity, $\Pi_c \geq 0 \iff q_p \geq \bar{q}_c$.

\textit{Step 4: Signal-contingent implementation.}
The consumer does not observe $q_p$ directly but instead conditions on the consensus signal $Z$. The threshold policy is implemented through Bayesian updating: when the consumer observes $Z = 1$, its posterior $f(q_p \mid Z = 1)$ upweights high-quality producers (by the MLRP), and the condition $\mathbb{E}[V_c(q_p) \mid Z = 1] \geq C_c$ is automatically satisfied for producers whose true quality exceeds $\bar{q}_c$---because validation is informative enough to distinguish quality levels above and below the threshold in expectation.
\end{proof}
...
As a concrete example: with $n = 5$, $q_v = 0.8$, direct computation yields $P_{\text{match}}(5, 0.8) \approx 0.942$, so the left-hand side of \eqref{eq:ic} becomes $(\tau_{\text{val}} + 2\sigma) \times 0.442$. Setting $\tau_{\text{val}} = \sigma = 1$ gives $3 \times 0.442 = 1.326$, which exceeds any effort cost $c_u < 1$.
...
\textit{Consumers}: Given that validators play truthfully (making $Z$ informative) and producers self-screen (so that only agents with $q_p \geq \bar{q}_p$ promote), the consumer faces a pre-screened pool of producers. The Bayesian posterior (Equation~\ref{eq:posterior}) correctly incorporates both the validation signal and the producer's self-screening. By Theorem~\ref{IR-based Policy for Consumers}, subscribing when $\mathbb{E}[V_c(q_p) \mid Z = 1] \geq \tau_s + mn\tau_{\text{val}}$ maximizes the consumer's expected payoff.
...
\begin{proof}
The proof structure mirrors Theorem~\ref{Strict IC for Validators}; only the matching probability changes. The honest validator $v_i$'s payoff structure remains identical: it stakes $\sigma$, earns $R_{\min}$ if it matches the majority, and forfeits $\sigma$ otherwise. The effort cost $c_u$ and the shirking payoff $\frac{1}{2}(R_{\min} - \sigma)$ are unchanged (a random vote still matches any majority with probability $\frac{1}{2}$, regardless of the adversary's presence, because the random vote is independent of all other votes).

It remains to show $P_{\text{match}}^{\text{Sybil}} > \frac{1}{2}$. Condition on the ground truth $\theta$. When $\theta$ agrees with the adversary's vote direction (e.g., both say ``pass''), the adversary's $k$ votes reinforce truth, and the honest majority is even more likely to prevail---so $P_{\text{match}}^{\text{Sybil}} \geq P_{\text{match}}$ in this case. When $\theta$ opposes the adversary (e.g., truth is ``fail'' but adversary votes ``pass''), the honest validators must overcome the $k$ adversarial votes. Since $h > n/2$, the honest bloc has more seats than needed. By the Condorcet jury theorem applied to the $h$ honest validators (each with accuracy $q_v > \frac{1}{2}$), their majority is correct with probability exceeding $\frac{1}{2}$. A truthful $v_i$ agrees with this correct honest majority with probability at least $q_v^2 + (1-q_v)^2 > \frac{1}{2}$.

Averaging over both cases (weighted by $\Pr(\theta)$), we obtain $P_{\text{match}}^{\text{Sybil}} > \frac{1}{2}$. Substituting into the IC derivation (Equations~\eqref{eq:u_truthful}--\eqref{eq:ic_derivation}) yields condition~\eqref{eq:ic_sybil}.
\end{proof}
...
Since $q_v^2 + (1 - q_v)^2 > \frac{1}{2}$ for all $q_v \neq \frac{1}{2}$ and $P_{\text{match}}(n, q_v) > \frac{1}{2}$ by Remark~\ref{rem:pmatch}(i), the convex combination satisfies $P_{\text{match}}^{\rho} > \frac{1}{2}$ for all $\rho \in [0, 1)$ and $q_v > \frac{1}{2}$.
