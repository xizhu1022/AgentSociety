...
We now establish the tighter bound. Let $p_{\text{maj}} = \Pr(\text{majority correct})$. By the Condorcet jury theorem applied to the full committee, $p_{\text{maj}} \geq q_v > \tfrac{1}{2}$. Decomposing $P_{\text{match}}$ by whether $v_i$ and the majority are correct gives
\[
P_{\text{match}} = \Pr(v_i \text{ correct}, \, \text{maj correct}) + \Pr(v_i \text{ incorrect}, \, \text{maj incorrect}).
\]
Since both events are monotone functions of the same underlying report profile conditioned on the ground truth $\theta$, the indicators $\mathbf{1}\{v_i \text{ correct}\}$ and $\mathbf{1}\{\text{maj correct}\}$ are positively correlated, which yields
\[
\Pr(v_i \text{ correct}, \, \text{maj correct}) \geq q_v \cdot p_{\text{maj}}, \qquad \Pr(v_i \text{ incorrect}, \, \text{maj incorrect}) \geq (1-q_v)(1-p_{\text{maj}}).
\]
Summing gives $P_{\text{match}} \geq q_v\,p_{\text{maj}} + (1-q_v)(1-p_{\text{maj}})$. A direct calculation shows
\[
q_v\,p_{\text{maj}} + (1-q_v)(1-p_{\text{maj}}) - \bigl[q_v^2 + (1-q_v)^2\bigr] = (2q_v - 1)(p_{\text{maj}} - q_v) \geq 0,
\]
since $q_v > \tfrac{1}{2}$ and $p_{\text{maj}} \geq q_v$. Therefore $P_{\text{match}} \geq q_v^2 + (1-q_v)^2 > \tfrac{1}{2}$, where the final strict inequality uses $q_v^2 + (1-q_v)^2 = 1 - 2q_v(1-q_v) > \tfrac{1}{2}$ for $q_v \neq \tfrac{1}{2}$.
...
\textit{Step 2: Boundary behavior.} At $q_p = \frac{1}{2}$, the per-task agreement probability is $p_{\text{agree}}(\frac{1}{2}, q_v) = \frac{1}{2}q_v + \frac{1}{2}(1-q_v) = \frac{1}{2}$ for any $q_v$, so each validator votes ``match'' with probability $\frac{1}{2}$, and by symmetry of majority voting, $V(\frac{1}{2}) = \frac{1}{2}$. Therefore $\Pi_p(\frac{1}{2}) = \frac{1}{2}(\tau_s - c_s) - (\tau_p + mn\tau_{\text{val}}) < 0$ by the upper bound in Assumption~\ref{assum:costs}.

At $q_p = 1$, each validator reports ``match'' with probability $q_v$ independently, and by the Condorcet jury theorem (Proposition~\ref{prop:pmatch_properties}) the majority vote satisfies $V(1) \geq q_v$. Therefore $\Pi_p(1) \geq q_v(\tau_s - c_s) - (\tau_p + mn\tau_{\text{val}}) > 0$ by the lower bound in Assumption~\ref{assum:costs}.
