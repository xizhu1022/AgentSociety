...
\textit{Step 2: Boundary behavior.} At $q_p = \frac{1}{2}$, the per-task agreement probability is $p_{\text{agree}}(\frac{1}{2}, q_v) = \frac{1}{2}q_v + \frac{1}{2}(1-q_v) = \frac{1}{2}$ for any $q_v$, so each validator votes ``match'' with probability $\frac{1}{2}$, and by symmetry of majority voting, $V(\frac{1}{2}) = \frac{1}{2}$. Therefore $\Pi_p(\frac{1}{2}) = \frac{1}{2}(\tau_s - c_s) - (\tau_p + mn\tau_{\text{val}}) < 0$ by the entry-cost condition in Assumption~\ref{assum:costs}. At $q_p \to 1$, $V(q_p) \to 1$, so $\Pi_p(1) = (\tau_s - c_s) - (\tau_p + mn\tau_{\text{val}}) > 0$; the cost ordering $\tau_p \ll \tau_s$ ensures this inequality for any $c_s < \tau_s - \tau_p - mn\tau_{\text{val}}$, which holds under the entry-cost condition.
